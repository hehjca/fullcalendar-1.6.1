...
function Calendar(element, options, eventSources) {
	...
	function changeView(newViewName) {
		if (!currentView || newViewName != currentView.name) {
			...
			if (t.options.viewChanged) {
			    t.options.viewChanged(currentView, oldView);
			}
		}
	}
...
function ListView(element,calendar) {
    ...
    //import
    //BasicView.call(t, element, calendar, 'list');
    //var renderBasic = t.renderBasic;

    function render() {
        $('td.fc-header-left').hide();
        $('td.fc-header-center').hide();

        //renderBasic(1, 1, false);
    }
    function unselect() {
        $('td.fc-header-left').show();
        $('td.fc-header-center').show();
    }
    function setHeight() {
    }
    function setWidth() {
